...
export default class UserController {
  ...
  async share({ request, response, view, session }: HttpContextContract) {
    try {
      const all = request.all()
      let user = await Database.from('customer').select('id', 'user_id', 'relation_user_id', 'relation', 'relation_log_id', 'introduction').where({ id: all.id, status: 1 }).whereNull('deleted_at').orderBy('created_at').first() || {}

      if (user.relation_log_id) {
        user.parent = await Database.from('users').select('user_id', 'nickname', 'avatar_url', 'company', 'job_title').where('user_id', user.user_id).first() || {}
        user = {
          ...user,
          ...await Database.from('customer_log').select('nickname', 'avatar_url', 'sex', 'height', 'weight', 'photos', 'mbti', 'location', 'birthday', 'work').where({ id: user.relation_log_id }).first() || {}
        }
      } else if (user.relation_user_id) {
        user.parent = await Database.from('users').select('user_id', 'nickname', 'avatar_url', 'company', 'job_title').where('user_id', user.relation_user_id).first() || {}
        user = {
          ...user,
          ...await Database.from('users').select('nickname', 'avatar_url', 'sex', 'height', 'weight', 'photos', 'mbti', 'location', 'birthday', 'work').where({ user_id: user.relation_user_id }).first() || {}
        }
      }
  ...
  async incentiveLogin({ request, response, view, session }: HttpContextContract) {
    try {
      const all = request.all()
      console.log(all);
