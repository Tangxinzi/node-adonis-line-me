...
export default class AliyunController {
  public async SendSms({ response, request, session }: HttpContextContract) {
    try {
      ...
      let sms = await Database.from('sms').where({ phone: all.PhoneNumbers, user_id: session.get('user_id') || null }).orderBy('created_at', 'desc').first() || {}
      if (sms.id) {
      ...
      client.sendSMS(params).then(async (result) => {
        console.log(result);
        await Database.table('sms').insert({ user_id: session.get('user_id') || null, phone: all.PhoneNumbers, code, result: JSON.stringify(result), ip: request.ip() })
        return { errno: 1, message: result }
      }, async (ex) => {
        console.error(ex);
        await Database.table('sms').insert({ user_id: session.get('user_id') || null, phone: all.PhoneNumbers, code, result: JSON.stringify(ex), ip: request.ip() })
