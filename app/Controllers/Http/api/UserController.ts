  ...
  public async wxaLogin({ request }: HttpContextContract) {
    try {
      ...
      if (!result.user.id) {
        ...
        const id = await Database.table('users').returning('id').insert({ user_id, wechat_open_id: result.openid, ip: request.ip() })
        result.user.id = id[0]
    ...
    try {
      ...
      if (user) {
        ...
        user['number'] = {
          ...
          introducer: (await Database.from('customer').whereIn('status', [1, 2, 3]).where({ user_id: user_id }).count('* as total'))[0].total,
        }
    ...
    try {
      ...
      switch (all.type) {
        ...
        case 'weight':
          ...
          break;
        case 'work':
          await Database.from('users').where('user_id', session.get('user_id')).update({ work: JSON.stringify(all.value || ''), work_code: all.value ? all.value.code : '' })
          break;
        case 'birthday':
          await Database.from('users').where('user_id', session.get('user_id')).update({ birthday: all.value })
          break;
