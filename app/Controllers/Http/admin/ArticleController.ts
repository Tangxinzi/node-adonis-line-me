...
export default class ArticleController {
  ...
  public async catalog({ params, request, view, response }: HttpContextContract) {
    try {
      ...
      if (all.search) {
        ...
        var data = await Database.from('articles').select('id', 'article_catalog', 'article_title', 'article_author', 'article_detail', 'article_theme_url', 'article_original_url', 'target', 'created_at').where({ status: 1, article_catalog: params.catalog }).orderBy('created_at', 'desc').forPage(request.input('page', 1), 8)
      }
  ...
  public async save({ request, response, session }: HttpContextContract) {
    try {
      ...
      if (request.method() == 'POST' && all.button == 'update') {
        await Database.from('articles').where('id', all.id).update({ status: all.status, target: all.target, article_catalog: all.catalog, article_title: all.title, article_author: all.author, article_detail: all.detail, article_original_url: all.original_url, article_theme_url })
        session.flash('message', { type: 'success', header: '更新成功', message: `` })
        return response.redirect().toRoute('admin/ArticleController.edit', { id: all.id }) // return response.redirect('back')
      }
      ...
      return response.redirect().toRoute('admin/ArticleController.edit', { id }) // return response.redirect('back')
    } catch (error) {
      console.log(error)
  ...
  public async delete({ session, request, response }: HttpContextContract) {
    try {
      const all = request.all()
      await Database.from('articles').where('id', all.id).update({ status: 0, deleted_at: Moment().format('YYYY-MM-DD HH:mm:ss') })
      session.flash('message', { type: 'success', header: '文章已删除成功！', message: `` })
      return response.redirect().toRoute('admin/ArticleController.index') // return response.redirect('back')
    } catch (error) {
      console.log(error);
    }
  }
